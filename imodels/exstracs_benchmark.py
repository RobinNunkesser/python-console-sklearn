...
Reuses dataset loading, aggregation, and CSV export from uciml.py.
Results can be combined with imodels results via plot_uciml_csvs.py:

    python exstracs_benchmark.py --no-show --output-dir results_exstracs
    python plot_uciml_csvs.py \\
        --input-csvs "results_imodels/uci_imodels_plot_data.csv,results_exstracs/exstracs_plot_data.csv" \\
        --no-show --output-dir merged_results
"""
...
# Reuse all shared utilities from uciml.py — no code duplication.
from uciml import (
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="ExSTraCS benchmark — connect results to imodels via plot_uciml_csvs.py"
    )
    parser.add_argument(
    ...
    parser.add_argument("--output-dir", default="results_exstracs")
...
def main() -> None:
    ...
    print(
        ...
        f"  python plot_uciml_csvs.py \\\n"
        f"    --input-csvs \"results/uci_imodels_plot_data.csv,"
        f"{args.output_dir}/exstracs_plot_data.csv\" \\\n"
        f"    --no-show --output-dir merged_results"
    )
